...
const vannstand = require("../models/vannstandModel");
...
const getVannstand = asyncHandler(async (req, res) => {
    ...
    const vannstandFound = await vannstand.find({}).sort({createdAt: -1 }); 
...
const updateVannstand = asyncHandler(async (req, res) => {
    ...
    const updatedVannstand = await vannstand.findByIdAndUpdate(req.params.id,
        req.body, {
            new: true,
        })
